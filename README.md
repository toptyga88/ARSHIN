...
**MIT** хранит карточки **утверждённых типов СИ** — зарегистрированных в Росстандарте моделей измерительных приборов. По каждому типу СИ известно: кто изготовитель, какие у него модификации (например, разные диаметры), какие PDF-документы прилагаются, какие межповерочные интервалы.
...
| `notation` | string | обозначение типа СИ. Например `СВК Ду 15-50` | Из списка не читаем — берём `notation` (массив) из карточки MIT |
...
|-------------|-----|---------------------------|-------------|
| `general.notation` | массив строк | обозначение типа СИ (`["ТВТ 1001", "ТВТ 1002"]`). У одного типа СИ может быть несколько обозначений | **Да.** Записываем в `dim_reference.обозначение` через запятую |
...
| `spec[].doc_url` / `meth[].doc_url` | string | ссылки на PDF-документы про тип СИ от изготовителя — где перечислены все модификации с диаметрами | **Да.** Главный источник диаметров. Скачиваем PDF, парсим, заполняем `dim_reference.модификация`. Сначала пробуем `spec`, если там нет — `meth` |
