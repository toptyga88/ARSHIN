...
[Термины](#-термины) · [Документация](#-документация) · [О проекте](#-о-проекте) · [Источник данных](#-источник-данных) · [Предметная область](#-предметная-область) · [Структура БД](#-структура-бд) · [Архитектура](#-архитектура) · [Запуск](#-запуск)
...
Типы и структуры данных первичны, алгоритмы вторичны. Сначала разбираемся какие данные отдаёт API и что каждое поле значит — потом пишем код. Структура README отражает тот же порядок.
...
| `manufacturer[].address` | string | адрес изготовителя. Регуляркой `г.<город>` вытаскиваем город → `dim_brands.город` |
| `spec[].doc_url` / `meth[].doc_url` | string | ссылки на PDF про тип СИ с перечнем модификаций. Скачиваем, парсим, заполняем `dim_reference.модификация`. Сначала `spec`, если пусто — `meth` |
...
| `mi_modification` | string | модификация поверенного экземпляра (`СВК-15`, `СВМ Ду 25`). Регуляркой вытаскиваем диаметр → определяем категорию |
| `org_title` | string | организация-поверитель |
| `verification_date` | date | дата поверки, формат ISO. Источник для разбиения объёмов по месяцам |
...
| 🤔 | **Нет признака «это изготовитель»** — определяем эвристически по доле `vriType=1` |
| 🔍 | **`vriType` только в карточке** `/vri/<vri_id>` — отсюда функция `ping_detail` |
| 📅 | **Два формата дат** — запрос принимает `YYYY-MM-DD`, ответ возвращает ISO `YYYY-MM-DDTHH:MM:SSZ` |

---

## 🧩 Предметная область

Сущности и связи реального мира — что мы моделируем.

```mermaid
erDiagram
    БРЕНД ||--o{ "ТИП СИ" : "включает"
    "ТИП СИ" }o--|| ПРОИЗВОДИТЕЛЬ : "изготавливается"
    "ТИП СИ" ||--o{ УСТРОЙСТВО : "экземпляры"
    "ТИП СИ" ||--o{ ПОВЕРКА : "регламентирует"
    УСТРОЙСТВО ||--o{ ПОВЕРКА : "проходит"
    ПОВЕРИТЕЛЬ ||--o{ ПОВЕРКА : "проводит"
    ПРОИЗВОДИТЕЛЬ ||..o{ ПОВЕРИТЕЛЬ : "может быть в роли (первичная поверка)"

    БРЕНД {
        string название "Декаст, Тепловодомер..."
        string страна
        string город
    }

    "ТИП СИ" {
        string рег_номер PK "12345-21"
        string наименование
        string обозначение "СВК Ду 15-50"
        string модификации "15, 20, 25, 40"
        string категория "бытовой/домовой/промышленный/тепло"
    }
    ...
    УСТРОЙСТВО {
        string заводской_номер PK "конкретный экземпляр"
        string модификация "СВК-15"
    }

    ПОВЕРКА {
        string vri_id PK
        date дата
        string vriType "1=первичная, 2=периодическая"
        bool пригоден
        date действительна_до
    }

    ПОВЕРИТЕЛЬ {
        string название "юрлицо-поверитель"
    }
```

- **Бренд** — наша группировка типов СИ для аналитики (один бренд → много типов СИ)
- **Тип СИ** — утверждённый в Росстандарте тип прибора с рег.номером
- **Производитель** — изготовитель типа СИ (из карточки MIT)
- **Устройство** — физический экземпляр прибора (заводской номер)
- **Поверка** — событие поверки одного устройства одним поверителем
- **Поверитель** — юрлицо проводившее поверку. Если совпадает с производителем → `vriType=1` (первичная поверка на заводе)

---

## 🗄 Структура БД

Четыре таблицы PostgreSQL. Каждый этап пайплайна пишет в свою таблицу. Все операции идемпотентны (`ON CONFLICT DO UPDATE`). Ручные правки сохраняются между прогонами.
...
| `бренд` | TEXT | бренд (денормализация для DataLens) |
