...
```mermaid
erDiagram
    БРЕНД ||--o{ "ТИП СИ" : "1 to N (включает)"
    "ТИП СИ" }o--|| ПРОИЗВОДИТЕЛЬ : "N to 1 (изготавливается)"
    "ТИП СИ" ||--o{ УСТРОЙСТВО : "1 to N (экземпляры)"
    "ТИП СИ" ||--o{ ПОВЕРКА : "1 to N (регламентирует)"
    УСТРОЙСТВО ||--o{ ПОВЕРКА : "1 to N (проходит)"
    ПОВЕРИТЕЛЬ ||--o{ ПОВЕРКА : "1 to N (проводит)"
    ПРОИЗВОДИТЕЛЬ ||..o{ ПОВЕРИТЕЛЬ : "1 to N (роль при первичной поверке)"
